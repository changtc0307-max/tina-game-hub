(()=>{
let bypass=false,active=false,finished=false,trap=false,finishedHash='';
...
function markActive(){if(active&&!finished)return;active=true;finished=false;finishedHash='';if(!trap){history.pushState({tinaGameGuard:true},'',location.href);trap=true}}
function markFinished(){finished=true;active=false;trap=false;finishedHash=location.hash}
...
window.addEventListener('popstate',()=>{if(bypass||!active||finished||!trap)return;trap=false;if(confirmLeave()){active=false;bypass=true;history.back();setTimeout(()=>bypass=false,80)}else{history.pushState({tinaGameGuard:true},'',location.href);trap=true}});
...
document.addEventListener('click',e=>{if(bypass||!active||finished||!gameVisible())return;const back=e.target.closest('.back');if(!back)return;e.preventDefault();e.stopImmediatePropagation();if(!confirmLeave())return;active=false;trap=false;bypass=true;try{location.hash=''}finally{setTimeout(()=>bypass=false,0)}},true);
...
const musicExtra=document.createElement('script');musicExtra.src='music-extra.js?v=20260915-6';musicExtra.onload=()=>{const lyrics=document.createElement('script');lyrics.src='lyrics-game.js?v=20260915-6';lyrics.onload=()=>{const extra=document.createElement('script');extra.src='lyrics-extra.js?v=20260915-6';extra.onload=()=>{const ui=document.createElement('script');ui.src='lyrics-ui.js?v=20260915-6';document.head.appendChild(ui)};document.head.appendChild(extra)};document.head.appendChild(lyrics)};document.head.appendChild(musicExtra);
const priceExtra=document.createElement('script');priceExtra.src='pricing-extra.js?v=20260915-6';document.head.appendChild(priceExtra);
window.addEventListener('hashchange',()=>{const q=(location.hash||'').slice(1);if(!['fahrenheit','sasha'].includes(q)||typeof MUSIC_GAMES==='undefined'||!MUSIC_GAMES[q])return;currentGame=q;resetUI();home.style.display='none';game.style.display='flex';musicGame(q)});
