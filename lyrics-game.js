...
function startLyricGame(artist,type,pk){const label=type==='guess'?'看歌詞猜歌':'歌詞填空',b=lyricBoard(lyricArtistTitle(artist)+label+(pk?' PK':'')),qs=lyricPick5(artist);let round=1,player=0,scores=pk?[0,0]:[0],aAns='',fill=null,locked=false;function prep(){fill=type==='fill'?makeFill(qs[round-1]):null}function render(){locked=false;const q=qs[round-1],prompt=type==='guess'?'「'+q.line+'」':'「'+fill.prompt+'」';b.innerHTML=lyricInput(prompt,round,pk?'玩家 '+(player?'B':'A'):'單人',scores[player]||0)+(type==='fill'?'<div style="font-size:12px;color:#7b8288;text-align:center;margin-top:12px">只填空格中的文字，不需要輸入任何標點符號。</div>':'');const input=document.getElementById('lyricInput'),btn=document.getElementById('lyricSubmit');btn.addEventListener('click',submit);input.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();submit()}});input.focus()}function correct(raw){const q=qs[round-1];if(type==='guess'){const aliases=MUSIC_GAMES[artist]?.aliases?.[q.song]||[];return[q.song,...aliases].some(x=>lyricN(x)===lyricN(raw))}return lyricN(raw)===lyricN(fill.answer)}function submit(){if(locked)return;const input=document.getElementById('lyricInput');if(!input)return;const raw=input.value.trim();if(!raw)return;locked=true;const q=qs[round-1];if(pk&&player===0){aAns=raw;b.innerHTML='<div style="text-align:center;padding:48px 10px"><div style="font-size:44px">🙈</div><h2>玩家 A 已送出</h2><p style="color:#68717a">答案已隱藏，請把裝置交給玩家 B。</p><button id="lyricHandoff" class="go" style="width:100%;max-width:400px">交給玩家 B</button></div>';document.getElementById('lyricHandoff').onclick=()=>{player=1;render()};return}const bp=correct(raw)?100:0,ap=pk?(correct(aAns)?100:0):null;if(pk){scores[0]+=ap;scores[1]+=bp}else scores[0]+=bp;const answer=type==='guess'?q.song:fill.answer;b.innerHTML=`<div style="text-align:center"><b>第 ${round}/5 題結果</b><div style="font-size:23px;font-weight:750;line-height:1.7;margin:22px auto;max-width:650px">${type==='guess'?'「'+q.line+'」':'「'+fill.prompt+'」'}</div><div style="font-size:14px;color:#68717a">正確答案</div><div style="font-size:28px;font-weight:800;margin:7px 0 20px">${answer}</div>${type==='fill'?'<div style="font-size:13px;color:#68717a;margin:-10px 0 18px">出處：'+q.song+'</div>':''}${pk?`<div style="display:grid;grid-template-columns:1fr 1fr;gap:9px;max-width:480px;margin:auto"><div style="background:#f0f2f4;border-radius:12px;padding:14px"><b>玩家 A</b><div>${aAns}</div><b>${ap?'+100':'0'} 分</b></div><div style="background:#f0f2f4;border-radius:12px;padding:14px"><b>玩家 B</b><div>${raw}</div><b>${bp?'+100':'0'} 分</b></div></div><div style="margin:15px;font-weight:700">累積　A ${scores[0]}：${scores[1]} B</div>`:`<div style="background:#f0f2f4;border-radius:12px;padding:14px;max-width:430px;margin:auto">你的答案：${raw}<br><b>${bp?'+100':'0'} 分</b><br>累積 ${scores[0]} 分</div>`}<button id="lyricNext" class="go" style="width:100%;max-width:430px;margin-top:18px">${round===5?'查看最終結果':'下一題'}</button></div>`;document.getElementById('lyricNext').onclick=()=>{if(round===5)return end();round++;player=0;aAns='';prep();render()}}function end(){if(window.TinaGuard)TinaGuard.markFinished();const text=pk?(scores[0]===scores[1]?'平手！':'玩家 '+(scores[0]>scores[1]?'A':'B')+' 勝！'):'挑戰完成！';b.innerHTML=`<div style="text-align:center;padding:40px 10px"><div style="font-size:44px">🎵</div><h2>${text}</h2><div style="font-size:30px;font-weight:800;margin:20px">${pk?'A '+scores[0]+'：'+scores[1]+' B':scores[0]+'/500'}</div><button id="lyricAgain" class="go" style="width:100%;max-width:400px">再玩一局</button></div>`;document.getElementById('lyricAgain').onclick=()=>startLyricGame(artist,type,pk)}if(qs.length<5){b.innerHTML='<p>此題庫目前不足 5 首可用歌曲。</p>';return}if(window.TinaGuard)TinaGuard.markActive();prep();render()}
