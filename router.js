...
function titleFits(route){const t=(document.getElementById('title')?.textContent||'').trim();const names={countries:'世界國家',capitals:'世界首都',japan:'日本',us:'美國',taiwan:'台灣',china:'中國',aov:'傳說對決',831:'八三夭',bestards:'理想混蛋',cosmos:'宇宙人',samlee:'李聖傑',fahrenheit:'飛輪海',sasha:'Sasha Alex Sloan',price:'價格猜猜看',timeline:'時間線'};let base=route.replace(/^pk-/,'').replace(/-lyrics-(guess|fill)(-pk)?$/,'');const needle=names[base];return !needle||t.includes(needle)}
function verify(route){clearTimeout(repairTimer);repairTimer=setTimeout(()=>{if(q()!==route)return;if(!route){if(gameVisible())run('',true);return}if(owned(route)&&(!gameVisible()||!titleFits(route)))run(route,true)},150)}
